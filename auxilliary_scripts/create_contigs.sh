...
if [ "$blast9_flag" = 'blast9' ] ; then
    ...
    python3 ./merge_deseq_results.py $path
    ...
    # echo $blat_path
    # ./blat.sh $blat_path $path/A_contigs.fasta $path/A_contigs_alignment.psl "blast9"
    # ./blat.sh $blat_path $path/B_contigs.fasta $path/B_contigs_alignment.psl "blast9"
else
    echo "blast9 flag is false: Creating BED file"
    echo $blat_path
    ./blat.sh $blat_path $path/A_contigs.fasta $path/A_contigs_alignment.psl.psl "BED"
    psl2bed < $path/A_contigs_alignment.psl.psl > $path/A_contigs_alignment.bed
    ./blat.sh $blat_path $path/B_contigs.fasta $path/B_contigs_alignment.psl.psl "BED"
    psl2bed < $path/B_contigs_alignment.psl.psl > $path/B_contigs_alignment.bed
fi

# echo "Merging results from dekupl and spades"
# python3 ./create_fastas.py $path
# echo $blat_path
# ./blat.sh $blat_path $path/A_contigs.fasta $path/A_contigs_alignment.psl
# ./blat.sh $blat_path $path/B_contigs.fasta $path/B_contigs_alignment.psl
