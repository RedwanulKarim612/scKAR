...
if [ "$merge_method" = "hmp" ]; then
  ...
  python fisherAndStouffer.py "$path/abyss/A_contigs.tsv" "$path/abyss/A_kmers.tsv" "$path/abyss/A_contigs_pvalues.tsv" "$merge_method"
  python fisherAndStouffer.py "$path/abyss/B_contigs.tsv" "$path/abyss/B_kmers.tsv" "$path/abyss/B_contigs_pvalues.tsv" "$merge_method"
